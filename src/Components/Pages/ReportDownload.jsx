...
function ReportDownload({ appointmentId, currentTest, endLoading, pdfLoading }) {
    ...
    const fetchAppointmentData = async () => {
        try {
            ...
            if (response.success) {
                ...
                fetchPatientProfile(response.data.patientId._id)
                setLoading(false)
            } else {
    ...
    const fetchPatientProfile = async (id) => {
        try {
            const response = await getSecureApiData(`patient/profile-detail/${id}`)
            if (response.success) {
                setProfiles(response.user)
            ...
            <div className="col-lg-12 col-md-12 col-sm-12 m-3" ref={reportRef}>
                <div className="new-invoice-card">
                    <div className="d-flex align-items-center justify-content-between mb-3">
                        ...
                        <div>
                            <button className="no-print print-btn" onClick={handleDownload}> <FontAwesomeIcon icon={faDownload} /> Download PDF</button>
                        </div>
                    </div>
                    ...
                    <div className="row">
                        ...
                        <div className="col-lg-6">
                            ...
                            {appointmentData?.labStaff && <div className="laboratory-bill-bx laboratory-sub-bx">
                                <h6 className="my-0">Lab Doctor</h6>
                                <h4 >{appointmentData?.labStaff?.name}</h4>
                            </div>}
                        </div>
                    </div>
