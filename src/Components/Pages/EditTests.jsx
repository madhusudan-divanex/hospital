...
function EditTests() {
  ...
  const testSubmit = async (e) => {
    ...
    const data = { ...testData, component: components, testId, type: 'hospital' ,sample:sampleData}
    try {
      const response = await updateApiData(`api/hospital/test`, data)
      if (response.success) {
  ...
  const getTestData = async () => {
    try {
      const response = await getSecureApiData(`api/hospital/test-data/${testId}`)
      if (response.success) {
  ...
  const validateTestData = () => {
    ...
    if (testData.precautions && testData.precautions.length > 500) {
      errors.precautions = "Max 500 characters allowed";
    }
      ...
      <div className="main-content flex-grow-1 p-3 overflow-auto">
        <div className="row mb-3">
          <div className="d-flex align-items-center justify-content-between tp-sub-main-bx">
            <div>
              <h3 className="innr-title mb-2">Edit Test </h3>
